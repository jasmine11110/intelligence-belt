...
export default class AbstractBlueTooth extends IBLEOperator {
    ...
    async createBLEConnection({deviceId, valueChangeListener}) {
        ...
        const self = this;
        // 官方建议：回调必须永远为函数，不能为 undefined
        onBLECharacteristicValueChange(typeof valueChangeListener === 'function' ? function(res) {
            console.log('接收到消息', res);
            const {value, protocolState, filter} = self.dealReceiveData({receiveBuffer: res.value});
